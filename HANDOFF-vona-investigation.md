...
## 1. The headline finding (established, gate-passed)
...
- Championship delta **+8.50%, 95% CI [7.02%, 9.98%]**, positive in **6/7** seasons
- 2,800 fresh-seed paired rooms per policy, all seasons 2018–2024
- Report: `out/draft_historical_robustness_consensus.md`
- This **passes** the gate (`lo > 0` and `positive >= 5`) that `core_2` failed
  at 4/7 and `sd_proxy` failed at +0.11%
...
Consensus wins **with the worse signal**. The defect is in selection, not ranking.
...
Negative = took the player before the market would. +4.5 actual points per
player × 14 ≈ +63/season, which matches the points deltas in winning seasons
(+38 to +63). VONA reaches in nearly every round, worst in rounds 5, 7, 8, 12
(−5 to −7 picks).
...
**No `sd` definition affects championship rate.** Tested via the `sd_mode` knob
...
number computed across the whole eligible pool is a measurement artifact
dominated by undraftable players.
...
RB 31+ at −23 is the one live blind spot: our `sd` is 4-source *disagreement*,
so four sources agreeing "he's the backup" yields a small sd and the model cannot
see handcuff bimodality. ECR can — `ecr_min`/`ecr_max`/`ecr_sd` are already stored
